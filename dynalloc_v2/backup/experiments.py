...
def _diagnostic_hedge_signal(strategy_internal: str, backend: str, trainer: Any, costates: Any, proj_debug: dict[str, Any], n_assets: int) -> np.ndarray:
    zeros = np.zeros(int(max(n_assets, 0)), dtype=float)
    if strategy_internal == 'ppgdpo':
        return np.asarray(proj_debug.get('hedge_signal', zeros), dtype=float)
    if strategy_internal == 'pgdpo' and str(backend).lower() == 'pipinn' and trainer is not None and costates is not None:
        cross = getattr(getattr(trainer, 'env', None), 'C_train', None)
        if cross is not None:
            try:
                signal = np.asarray(cross, dtype=float) @ np.asarray(costates.JXY, dtype=float)
...
def _strategy_metadata(strategy: str, cross_mode: str, *, backend: str, market_source: str = '') -> dict[str, Any]:
    ...
    if strategy == 'pgdpo':
        if backend == 'pipinn':
            return {
                'strategy': 'pipinn_frozen_traincov',
                'strategy_display': 'pipinn_frozen_traincov',
    ...
    if strategy == 'ppgdpo':
        if backend == 'pipinn':
            mapping = {
                'estimated': ('pipinn', 'ppgdpo', 'PI-PINN value-gradient projection with estimated cross'),
                'zero': ('pipinn_zero', 'ppgdpo_zero', 'PI-PINN value-gradient projection with zero cross'),
                'regime_gated': ('pipinn_regime_gated', 'ppgdpo_regime_gated', 'PI-PINN value-gradient projection with regime-gated cross'),
            }
            out_name, legacy_name, note = mapping.get(cross_mode, (f'pipinn_{cross_mode}', f'ppgdpo_{cross_mode}', 'PI-PINN projected strategy'))
            return {
                'strategy': out_name,
                'strategy_display': out_name,
        ...
        out_name, legacy_name, note = mapping.get(cross_mode, (f'ppgdpo_{cross_mode}', f'ppgdpo_{cross_mode}', 'Pontryagin projection candidate'))
        return {
            'strategy': out_name,
            'strategy_display': out_name,
            'strategy_legacy_label': legacy_name,
...
def _write_benchmark_notes(
    ...
    *,
    ...
    market_source: str,
    backend: str,
    emit_pipinn_frozen_traincov: bool,
) -> Path:
    backend = str(backend).lower()
    if backend == 'pipinn':
        strategy_label_map = {
        ...
        benchmark_roles = {
            'myopic': 'reference_only',
            'pipinn': 'method_candidate',
            'pipinn_zero': 'method_candidate',
            'pipinn_regime_gated': 'method_candidate',
        }
        notes = [
            'myopic is the reference-only mean-variance benchmark',
            'pipinn / pipinn_zero / pipinn_regime_gated are PI-PINN value-gradient projections evaluated with the live runtime covariance forecast',
        ]
        if emit_pipinn_frozen_traincov:
            strategy_label_map['pipinn_frozen_traincov'] = 'pgdpo'
            benchmark_roles['pipinn_frozen_traincov'] = 'method_diagnostic'
            notes.append('pipinn_frozen_traincov is a diagnostic trainer-default projection that uses frozen train-window covariance and estimated cross')
    else:
        strategy_label_map = {
            'myopic': 'predictive_static',
            'pgdpo': 'pgdpo',
        ...
        benchmark_roles = {
            ...
            'ppgdpo_zero': 'method_candidate',
            'ppgdpo_regime_gated': 'method_candidate',
        }
        notes = [
            'myopic is the reference-only mean-variance benchmark',
    ...
    payload = {
        'version': 'v57_backend_native_labels',
...
def _run_ppgdpo_experiment(cfg: Config) -> RunArtifacts:
    ...
    for i, date_t in enumerate(eval_iterable):
        ...
        if refit_now:
            ...
            if show_progress and hasattr(eval_iterable, 'set_postfix_str'):
                eval_iterable.set_postfix_str(f"refit {refit_counter}: {pd.Timestamp(train_dates[0]).strftime('%Y-%m')}→{pd.Timestamp(train_dates[-1]).strftime('%Y-%m')}")
        ...
        targets: dict[tuple[str, str], np.ndarray] = {}
        if rebalance_now:
            diagnostic_traincov_w: np.ndarray | None = None
            if backend == 'ppgdpo' or emit_pipinn_frozen_traincov:
                diagnostic_traincov_w = trainer.policy_weights(state_row)
            ...
            if diagnostic_traincov_w is not None:
                targets[('pgdpo', reference_cross_mode)] = diagnostic_traincov_w
        ...
        for (strategy_internal, cross_mode_internal), current_holdings in list(holdings.items()):
            if rebalance_now:
                weights_for_return = np.asarray(targets[(strategy_internal, cross_mode_internal)], dtype=float)
                turnover = float(np.abs(weights_for_return - current_holdings).sum())
            else:
                weights_for_return = np.asarray(current_holdings, dtype=float)
                turnover = 0.0
            gross = float(weights_for_return @ realized_ret)
            net = gross - tc * turnover
            proj_debug = last_proj_debug.get(cross_mode_internal, {'hedge_signal': np.zeros(returns.shape[1], dtype=float)})
            strategy_meta = _strategy_metadata(
                strategy_internal,
                cross_mode_internal,
                backend=backend,
                market_source=market_source,
            )
            hedge_signal = _diagnostic_hedge_signal(
                strategy_internal,
                backend,
                trainer,
                last_costates,
                proj_debug,
                len(weights_for_return),
            )
            monthly_rows.append({
                **protocol_meta,
                'decision_date': pd.Timestamp(date_t),
                ...
                **strategy_meta,
                ...
                'weights_json': json.dumps({asset: float(weight) for asset, weight in zip(returns.columns, weights_for_return)}),
                **last_meta,
                'hedge_signal_l2': float(np.linalg.norm(np.asarray(hedge_signal, dtype=float))),
                ...
                'refit_index': int(refit_counter) if refit_counter > 0 else np.nan,
            })
            holdings[(strategy_internal, cross_mode_internal)] = _drift_holdings(weights_for_return, realized_ret)

        for strategy_internal, cross_mode_internal in direct_strategy_slots:
    ...
    benchmark_notes_yaml = _write_benchmark_notes(
        ...
        market_source=market_source,
        backend=backend,
        emit_pipinn_frozen_traincov=emit_pipinn_frozen_traincov,
    )
